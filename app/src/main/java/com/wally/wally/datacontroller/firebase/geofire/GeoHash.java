...
public class GeoHash {
    ...
    private void validateArgs(double latitude, double longitude, int precision) {
        String error = null;
        if (precision < 1) {
            error = "Precision of GeoHash must be larger than zero!";
        }
        if (precision > MAX_PRECISION) {
            error = "Precision of a GeoHash must be less than " +
                    (MAX_PRECISION + 1) + "!";
        }
        if (!coordinatesValid(latitude, longitude)) {
            error = "Not valid location coordinates: [" +
                    latitude + " , " + longitude + "]";
        }
    ...
    public GeoHash(double latitude, double longitude, int precision) {
        validateArgs(latitude, longitude, precision);
    ...
    private boolean coordinatesValid(double latitude, double longitude) {
